...
export default function NavBar({ scrolled, onMenuOpen }: NavBarProps) {
  ...
  return (
    <>
      <style>{`
        .navbar-desktop { display: flex; gap: 0; align-items: center; position: relative; }
        .navbar-hamburger {
          display: none;
          background: none; border: none; cursor: pointer;
          color: #f7f0e6; padding: 4px;
          align-items: center; justify-content: center;
        }
        @media (max-width: 768px) {
          .navbar-desktop { display: none !important; }
          .navbar-hamburger { display: flex !important; }
          .navbar-root { padding: 16px 20px !important; }
        }
      `}</style>
      ...
      >
        ...
        <div className="navbar-desktop">
          ...
          {navItems.map((item, i) => (
            <a
              ...
              style={{
                ...
                padding: '6px 16px', transition: 'color 0.2s',
                fontFamily: "'DM Sans', sans-serif",
              }}
            >
              {item.label}
          ...
          <span style={{ width: 1, height: 18, background: 'rgba(212,168,90,0.2)', margin: '0 16px', flexShrink: 0 }} />

          <Link href="/login" style={{
            color: 'rgba(247,240,230,0.65)', fontSize: 12, letterSpacing: '0.08em',
          ...
          <Link href="/cadastro" style={{
            marginLeft: 12, padding: '9px 22px', fontSize: 11,
            letterSpacing: '0.1em', textTransform: 'uppercase', textDecoration: 'none',
            color: '#f7f0e6', background: 'linear-gradient(135deg, #c8714a, #a85a38)',
            borderRadius: 2, fontFamily: "'DM Sans', sans-serif", fontWeight: 600,
        ...
        {/* Hamburger mobile */}
        <button className="navbar-hamburger" onClick={onMenuOpen} aria-label="Abrir menu">
          <Menu size={24} />
        </button>
      </nav>
    </>
